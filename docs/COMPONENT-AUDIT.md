...
- NativeSelect는 패키지에서 제거하고 Select Form 예제로 대체. 기존 문서 링크는 유지.
- DateField/TimeField는 직접 입력 + 공통 팝업, NumberField는 공통 증감 버튼으로 통일. 기간 입력도 같은 컴포넌트를 재사용.
- ScrollArea의 양방향 드래그/키보드와 페이지·사이드바·표·팝업의 일반 overflow 스킨을 패키지 CSS에 통합. 사이트에서 별도 컴포넌트 외형을 덮어쓰지 않음.
- DataTable의 표시 열 메뉴를 Popover로 통일. 업로드의 취소된 form reset을 보존하고 OTP 오류를 인라인으로 연결.
- 내비게이션의 상위 버튼·링크, Menubar와 Toolbar를 36px 공통 높이로 정렬. 팝업의 여러 줄 설명 링크는 별도 유지.
- Pagination은 40px 번호 버튼·현재 페이지·생략 표시를 사용. 좁은 화면은 7슬롯, 데스크톱은 9슬롯 이내로 제한하고 양 끝 버튼 공간을 유지. 좁은 화면에서도 현재 페이지 양옆으로 직접 이동 가능.
- 문서 사이트는 전역 메뉴·대분류 사이드바·실행 예제/가이드/API·페이지 내부 목차로 재정리. 선택한 항목은 사이드바 안에서만 자동 노출하고 본문 위치는 건드리지 않음.
- 소개에 ‘우주선이 달에 착륙하고 문을 여니 치즈 달이었다’는 창작 서사를 추가. 공식 STARSHIP 브랜드 설명과 명확히 구분.
...
- 업로드 취소 검사는 고정 800ms 타이머 대신 실제 요청 시작 → 취소 → 늦은 응답 순서로 동기화. 느린 WebKit에서 테스트가 취소 버튼을 누르기 전에 업로드가 완료되던 경쟁 조건 제거.
- 추가 요청: 단일 행 컨트롤의 줄 높이를 normal로 통일하고 텍스트 상자의 수직 중앙을 검사. 본문/제목 leading을 분리하고 굵기 토큰 400/500/600/700 추가. 주요 API를 패키지 Badge로 교체하고 문서 제목·본문·보조 설명 크기 계층 정리.
...
| Select, Select Form                                                                                   | 키보드·옵션 선택, disabled 옵션, required/reset/제출, 외부 form 연결                                |
| NumberField, DateField, TimeField                                                                     | min/max/step, 초 단위·윤년·날짜 전용 시간대, 직접 입력·팝업 선택, 읽기 전용·비활성·외부 form·초기화 |
...
- `tests/catalog-behavior.spec.ts`: Drawer/BottomSheet/Popover, Collapsible/Stepper, Radio/Progress, 공통 입력.
- `tests/date-number-fields.spec.ts`, `time-controls.spec.ts`, `workplace.spec.ts`: React/Vue 입력과 조합 업무 폼의 검증·제출·초기화.
- `tests/scrollbars.spec.ts`: 양방향 손잡이 드래그, 키보드·휠, 테마 범위, 강제 색상·모션 감소.
- `tests/navigation-pagination.spec.ts`: React/Vue 번호 탐색·큰 페이지 수·모바일 범위·키보드·비활성·공통 높이.
- `tests/docs-site.spec.ts`: 문서 메뉴/제목 일치, 내부 목차, 사이드바 독립 스크롤, 모바일 검색·이동.
...
3. **기본 시스템 동작의 경계:** 업무 입력 팝업은 CHEESE로 구현하지만 파일 선택창·모바일 키보드·브라우저 자동완성·강제 색상 설정은 유지한다. 소비자가 직접 Input에 native type을 전달하면 기본 위젯을 사용할 수 있으므로 날짜/시간에는 전용 필드를 사용한다.
