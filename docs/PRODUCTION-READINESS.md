...
골드만 강조색으로 사용합니다. 기존 success/danger 토큰은 API 호환성을 위해 유지하되 중립색으로 매핑합니다. 오류·완료 의미는 문구·아이콘·ARIA로 전달합니다. 금색 포커스에는 중립색 대비선을 함께 표시합니다. 기본 입력은 하단 식별선, 대비 강화 모드는 전체 경계, 강제 색상 모드는 시스템 색상으로 구분합니다. 실제 보조 기술과 고대비 모드 수동 검증은 여전히 필요합니다.
